...
| `recipes/vlm-detect.py` | VLM-as-labeller object detection. Default `Qwen/Qwen2.5-VL-3B-Instruct` (T4 fits comfortably). Free-form `DETECT_PROMPT` → JSON bounding boxes. Output schema (`detections`, `raw_response`, `inference_info`) matches [`davanstrien/uv-scripts-for-ai/vlm-object-detection`](https://huggingface.co/datasets/uv-scripts/vlm-object-detection) so you can copy his prompts verbatim. Override `MODEL_ID=Qwen/Qwen2.5-VL-7B-Instruct` on L4/A100 for better quality. |
